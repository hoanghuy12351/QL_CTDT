...
export default function FacultyTable({
  ...
  const isAllCurrentPageSelected =
    rows.length > 0 && rows.every((faculty) => selectedSet.has(faculty.id));
        ...
        <table className="w-full min-w-[900px] table-fixed border-collapse text-left text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            {rows.map((faculty, index) => {
              const isSelected = selectedSet.has(faculty.id);
              ...
              return (
                ...
                >
                  <td className="px-3 py-3 text-center">
                    <input
                      type="checkbox"
                      aria-label={`Chọn khoa ${faculty.name}`}
                      checked={isSelected}
                      onChange={() => onToggleRow(faculty.id)}
                      className="size-4 rounded border-slate-300 text-brand-600 focus:ring-brand-200"
                    />
                  </td>
                  ...
                  <td className="px-3 py-3">
                    <span className="inline-flex max-w-full items-center rounded-md bg-slate-100 px-2 py-1 font-semibold text-slate-900">
                      <span className="truncate">{faculty.code}</span>
                    </span>
                  </td>
